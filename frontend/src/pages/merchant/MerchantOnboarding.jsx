...
export default function MerchantOnboarding() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-white tracking-tight">Merchant Onboarding</h1>
      ...
      {/* Stepper Header */}
      <div className="bg-surface-alt rounded-2xl shadow-card border border-surface-border p-4">
            ...
            return (
              <div
                ...
                className={`flex items-center space-x-3 p-3 rounded-xl transition-colors ${
                  isActive
                    ? "bg-brand-500/20 border border-brand-500/30 text-white font-semibold"
                    : isDone
      ...
      {step === 1 && (
        <Card className="space-y-6">
          <h2 className="text-lg font-semibold text-white border-b border-surface-border pb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2 rounded-lg border border-surface-border bg-surface text-white focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
      ...
      {step === 2 && (
        <Card className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* File Upload Box */}
            <div className="border-dashed border-2 border-surface-border rounded-2xl p-6 text-center flex flex-col items-center justify-center bg-surface">
              <div className="w-12 h-12 rounded-full bg-brand-500/20 text-brand-400 flex items-center justify-center font-bold text-xl mb-3">
                📦
              </div>
              <p className="text-sm font-medium text-white mb-1">Upload Product Catalog (JSON)</p>
              <p className="text-xs text-ink-400 mb-4">Drag and drop or choose file from your computer</p>
              <label className="cursor-pointer">
                <span className="px-4 py-2 bg-surface-alt border border-surface-border rounded-lg text-xs font-medium text-white shadow-sm hover:bg-surface-border inline-block">
                  Choose JSON File
                </span>
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                className="w-full p-3 rounded-lg border border-surface-border bg-surface font-mono text-xs text-white focus:outline-none focus:ring-2 focus:ring-brand-500"
              />
          ...
          {uploadError && <div className="p-3 bg-danger-dark/40 border border-danger/30 text-danger text-xs rounded-lg">{uploadError}</div>}
          ...
          <div>
            ...
            <div className="overflow-x-auto border border-surface-border rounded-xl">
              <table className="w-full text-left text-xs">
                <thead className="bg-surface text-ink-400 uppercase font-medium border-b border-surface-border">
                  <tr>
                ...
                <tbody className="divide-y divide-surface-border font-sans text-ink-700">
                  {catalogItems.map((item, idx) => (
                    <tr key={idx} className="hover:bg-surface-border/40">
                      ...
                      <td className="p-3 text-right font-mono text-brand-500">
                        {formatRupee(item.minPriceInPaise || (item.minPrice || item.price) * 100)}
                      </td>
                      <td className="p-3 text-right font-mono text-white">{formatNumber(item.inventory)} units</td>
      ...
      {step === 3 && (
        <Card className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-surface p-5 rounded-xl border border-surface-border space-y-3">
              <h3 className="text-sm font-semibold text-ink-400">Merchant Identity</h3>
              <div className="text-2xl font-bold text-white">{merchantName}</div>
            ...
            <div className="bg-surface p-5 rounded-xl border border-surface-border space-y-3">
              <h3 className="text-sm font-semibold text-ink-400">Constitution Policy Summary</h3>
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <span className="text-ink-400 block">Max Discount:</span>
                  <span className="font-semibold text-white">{maxDiscountPct}%</span>
                </div>
                <div>
                  <span className="text-ink-400 block">Min Margin:</span>
                  <span className="font-semibold text-white">{formatRupee(minMarginRupees * 100)}</span>
                </div>
                <div>
                  <span className="text-ink-400 block">Max AI Tx Limit:</span>
                  <span className="font-semibold text-white">{formatRupee(maxAiTxRupees * 100)}</span>
                </div>
                <div>
                  <span className="text-ink-400 block">Refund Threshold:</span>
                  <span className="font-semibold text-white">{formatRupee(refundThresholdRupees * 100)}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {step === 4 && (
        <Card className="text-center py-10 space-y-4">
          <div className="w-16 h-16 bg-success-dark/40 border border-success/30 text-success rounded-full flex items-center justify-center text-3xl mx-auto font-bold">
            ✓
          </div>
          <h2 className="text-2xl font-bold text-white">Merchant Agent Activated!</h2>
          <p className="text-sm text-ink-400 max-w-md mx-auto">
            <strong className="text-white">{createdMerchant?.name}</strong> is now registered as an AI-transactable merchant with hard constitution policy locks enabled.
          </p>
          <div className="pt-4">
            ...
            <Card key={p._id} hoverable className="flex flex-col justify-between space-y-4">
              <div>
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[10px] font-mono uppercase bg-surface-border px-2 py-0.5 rounded text-brand-400 font-semibold">
              ...
              <div className="pt-3 border-t border-surface-border flex items-baseline justify-between">
                <div>
                  ...
                  <span className="text-lg font-semibold font-mono text-white">{formatRupee(p.priceInPaise)}</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] text-ink-400 block uppercase tracking-wide">Floor Price</span>
                  <span className="text-xs font-mono font-medium text-brand-500">{formatRupee(p.minPriceInPaise)}</span>
                </div>
              </div>
            </Card>
          ))}
