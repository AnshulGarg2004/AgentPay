...
export default function BuyerDashboard({ onSelectTransaction }) {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Buyer Procurement & Governance Portal</h1>
      ...
      <Card className="space-y-4">
        <div className="flex items-center justify-between border-b border-surface-border pb-3">
          <h3 className="text-base font-bold text-white">Procurement Orders & Negotiations</h3>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {transactions.map((txn) => (
              <div
                key={txn._id}
                className="p-4 bg-surface rounded-xl border border-surface-border space-y-3 hover:border-brand-500/40 transition-all"
              >
                ...
                <div>
                  <h4 className="text-sm font-bold text-white">{txn.productId?.name || "Product"}</h4>
                ...
                <div className="flex items-center justify-between border-t border-surface-border pt-2 text-xs">
                  <div>
                    ...
                    <span className="font-mono font-bold text-brand-500">{formatRupee(txn.amountInPaise)}</span>
                  </div>

                  {onSelectTransaction && (
                    <button
                      onClick={() => onSelectTransaction(txn._id)}
                      className="px-3 py-1 bg-surface-alt border border-surface-border rounded-lg text-brand-500 font-semibold hover:bg-surface-border hover:text-white text-xs shadow-sm"
                    >
                      View Details →
                    </button>
                  )}
